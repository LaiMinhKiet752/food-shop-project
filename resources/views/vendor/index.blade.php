...
<div class="page-content">
    ...
    @php
        ...
        $sales_today = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
        ...
        $get_order_month = \App\Models\Order::where('order_month', '=', $month)
            ...
            ->get('id');
        // dd($get_order_month);
        $sales_month = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
        ...
        $get_order_year = \App\Models\Order::where('order_year', $year)
            ...
            ->get('id');
        // dd($get_order_year);
        $sales_year = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
        ...
        $pending_orders_get_id = \App\Models\Order::where('status', 'pending')
            ->where('cancel_order_status', 0)
            ->get('id');
        // dd($pending_orders_get_id);

        $pending_orders = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
            ->whereIn('order_id', $pending_orders_get_id)
            ->distinct('order_id')
            ->count('order_id');
        ...
        $success_orders_get_id = \App\Models\Order::where('status', 'delivered')
            ...
            ->where('return_order_status', 0)
            ->get('id');
        // dd($success_orders_get_id);
        $success_orders = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
            ->whereIn('order_id', $success_orders_get_id)
            ->distinct('order_id')
            ->count('order_id');
        ...
        $cancel_orders = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
            ->whereIn('order_id', $cancel_orders_get_id)
            ->distinct('order_id')
            ->count('order_id');
        ...
        $return_orders = \App\Models\OrderDetails::where('vendor_id', $vendor_id)
            ->whereIn('order_id', $return_orders_get_id)
            ->distinct('order_id')
            ->count('order_id');
    @endphp
        ...
        <div class="col">
            <div class="card radius-10 bg-gradient-deepblue">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center text-white">
                        <p class="mb-0">TOTAL ORDER CANCELLED</p>
    ...
    @php
        $orders_pending_get_id = \App\Models\Order::where('status', 'pending')
            ->where('cancel_order_status', 0)
            ->orderBy('id', 'DESC')
            ->limit(10)
            ->get('id');

        // dd($orders_pending_get_id);
        $orders = \App\Models\OrderDetails::with('order')
            ->where('vendor_id', $vendor_id)
            ->whereIn('order_id', $orders_pending_get_id)
            ->select('order_id')
            ->groupBy('order_id')
            ->orderBy('id', 'DESC')
            ->limit(10)
                    ...
                    <tbody>
                        @foreach ($orders as $key => $order)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                @php
                                    $order_date = strtotime($order->order->order_date);
